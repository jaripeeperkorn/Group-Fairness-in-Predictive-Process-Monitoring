...
for lam in lambdas:
    model, loss_history, val_loss_history = train_model.train_and_return_LSTM(X_train = X_train, y_train= y_train, s_train=s_train,
                                                        loss_function = 'diff_quadr', 
                                                        vocab_sizes = vocsizes, 
                                                        num_numerical_features=num_numerical_features, 
                                                        dropout = 0.2, lstm_size = 32, num_lstm=1, max_length=8, learning_rate=0.0005, 
                                                        max_epochs = 300, batch_size=128,
                                                        patience = 30,
                                                        get_history=True, X_val=X_val, y_val=y_val, s_val=s_val,
