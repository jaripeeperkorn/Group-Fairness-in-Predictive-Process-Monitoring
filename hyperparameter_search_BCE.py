            ...
            for batch_size in batch_size_lst:
                for learnig_rate in learning_rate_lst:
                    for dropout in dropout_lst:
                            model = train_model.train_and_return_LSTM(X_train = X_train, y_train= y_train, s_train=s_train, 
                                                                      loss_function = 'BCE', vocab_sizes = vocsizes, 
                                                                      num_numerical_features=num_numerical_features, 
                                                                      dropout = dropout, lstm_size = lstm_size, num_lstm=num_layers, max_length=8, 
